...
const { sendMail, sendMailPassword } = require("../mailer");
...
router.post("/api/user/forgotpassword", (req, res) => {
  var { mail } = req.body;
  mail = xss(mail);
  if (!mail) {
    res.status(200).json({ message: "Required fields" });
  ...
  db.knex
    ...
    .then((data) => {
      if (data.length > 0) {
        var code = crypto.randomBytes(20).toString("hex");
        var link = "https://timtalctf.com/resetpassword/" + code + "/" + mail;
        sendMailPassword(mail, "Parola Sıfırlama", link);
        db.knex("forgot_password")
          .insert({
            mail: mail,
...
router.post("/api/user/resetpassword", (req, res) => {
  var { code, mail, password } = req.body;
